...
void init_spi1_gpio(uint8_t isMisoUsed, uint8_t isNssUsed) {
  ...
  GPIOA->MODER &= ~(GPIO_MODER_MODE5 | GPIO_MODER_MODE7);                    // clear default mode bits
...
void __base_init_spi1_fullDuplex(SPI_CPOL cpol, SPI_CPHA cpha, SPI_BR baudRate, SPI_MODE mode, SPI_SBFIRST sbFirst, SPI_DFF dff, SPI_SSM ssm) {
